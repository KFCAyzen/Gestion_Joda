...
import { FolderOpen, House, MessageSquare, User, WalletCards, type LucideIcon } from 'lucide-react-native';
...
const ICONS: Record<string, { icon: LucideIcon; label: string }> = {
  ...
  dossiers: { icon: FolderOpen, label: 'Dossiers' },
  paiements: { icon: WalletCards, label: 'Paiements' },
...
export function StaffTabs({ state, navigation }: TabBarProps) {
  ...
  const badgeFor = (name: string): number | null => {
    if (name === 'paiements') return badges?.payments || null;
    if (name === 'messages') return badges?.messages || null;
    return null;
  };
