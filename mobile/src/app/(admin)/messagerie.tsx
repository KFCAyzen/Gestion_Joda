  ...
  return (
    <ScreenBackground>
      <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
        <ScreenHeader eyebrow={`${unread} non lus`} title="Messagerie" sm onBack={() => router.back()} />
        ...
        {isLoading ? (
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
            {list.length ? (
              <ListCard>
                {list.map((t, i, arr) => (
                  <ListRow key={t.userId} last={i === arr.length - 1} onPress={() => router.navigate(`/(admin)/chat/${t.userId}?name=${encodeURIComponent(t.name)}` as Href)}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={styles.rowBottom}>
                        <Text style={[T.t2, { flex: 1 }, t.unread ? styles.unread : null]} numberOfLines={1}>{t.last}</Text>
